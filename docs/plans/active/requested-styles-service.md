...
The next useful foundation is local service startup on demand, with comms-web as the first consumer. This is a moderate Avatars lifecycle feature plus a small consumer integration; it does not require the agent worker or Google authentication.
...
This document records the proposed next capability only. The current delivery adds Mixed dogs/cats and the general mixed-choice authoring convention. Future work should start from the on-demand service and update this plan as the contracts are verified. Independent review identified runtime containment, activation consistency, and phase-specific prerequisites; those contracts are included above.
