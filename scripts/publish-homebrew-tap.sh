...
source_repo=${AVATARS_SOURCE_REPO:-marcus/avatars}
tap_repo=${AVATARS_TAP_REPO:-marcus/homebrew-tap}
release_workflow=${AVATARS_RELEASE_WORKFLOW:-release.yml}
discovery_timeout=${AVATARS_RELEASE_DISCOVERY_TIMEOUT:-600}
poll_interval=${AVATARS_RELEASE_POLL_INTERVAL:-5}
...
tap_repo_url() {
  if [[ -n ${AVATARS_TAP_REPO_URL:-} ]]; then
    echo "$AVATARS_TAP_REPO_URL"
    return
  fi
...
check_prerequisites() {
  ...
  [[ $discovery_timeout =~ ^[1-9][0-9]*$ ]] ||
    die "AVATARS_RELEASE_DISCOVERY_TIMEOUT must be a positive integer"
  [[ $poll_interval =~ ^[1-9][0-9]*$ ]] ||
    die "AVATARS_RELEASE_POLL_INTERVAL must be a positive integer"
...
validate_formula() {
  ...
  local validation_tap="avatars-release/validation-$$-$RANDOM"
  ...
  (
    ...
    cp "$formula" "$validation_repo/Formula/avatars.rb"
    brew style --formula "$validation_tap/avatars"
    brew audit --strict --online --formula "$validation_tap/avatars"
  )
}
...
push_tap_commit() {
  ...
  for attempt in 1 2 3; do
    ...
    if ! cmp -s "$tap_dir/Formula/avatars.rb" "$expected_formula"; then
...
publish_tap() (
  ...
  mkdir "$formula_dir"
  expected_formula="$formula_dir/avatars.rb"
  ...
  tap_formula="$tap_dir/Formula/avatars.rb"
  ...
  if [[ $transition_status == 10 ]]; then
    ...
    git -C "$tap_dir" add Formula/avatars.rb
    git -C "$tap_dir" diff --cached --check
    git -C "$tap_dir" commit -m "avatars $release_version"
    push_tap_commit "$tap_dir" "$expected_formula"
  fi

  git -C "$tap_dir" fetch origin main
  remote_commit=$(git -C "$tap_dir" rev-parse FETCH_HEAD)
  remote_formula="$temporary/remote-avatars.rb"
  git -C "$tap_dir" show "$remote_commit:Formula/avatars.rb" >"$remote_formula"
  ...
  echo "published and verified $tap_repo@$remote_commit: avatars $remote_version ($remote_sha)"
)
