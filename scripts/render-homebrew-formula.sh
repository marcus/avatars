...
usage() {
  echo "usage: $0 vX.Y.Z SHA256 OUTPUT/avatars.rb" >&2
  exit 2
}
...
[[ $(basename "$output") == avatars.rb ]] || {
  echo "formula output must be named avatars.rb" >&2
  exit 2
}
[[ -d $(dirname "$output") ]] || {
...
template="$repo_root/packaging/homebrew/avatars.rb.tmpl"
[[ -f $template ]] || {
  echo "formula template does not exist: $template" >&2
  exit 1
}

temporary=$(mktemp "$(dirname "$output")/.avatars.rb.XXXXXX")
cleanup() {
  rm -f "$temporary"
}
trap cleanup EXIT
