...
state_root=${AVATARS_DEV_STATE:-"$HOME/.local/state/avatars/dev-installs"}

die() {
  printf 'avatars dev install: %s\n' "$*" >&2
  exit 1
}

brew_prefix() {
  if [ -n "${AVATARS_BREW_PREFIX:-}" ]; then
    printf '%s\n' "$AVATARS_BREW_PREFIX"
  else
...
status() {
  ...
  resolved=$(command -v avatars 2>/dev/null || true)
  printf 'this shell resolves: %s\n' "${resolved:-not found}"
  if command -v zsh >/dev/null 2>&1; then
    interactive=$(zsh -lic 'whence -v avatars; avatars --version' 2>/dev/null || true)
...
install_local() {
  ...
  for name in $commands; do
    go build -ldflags "-s -w -X github.com/marcus/avatars/internal/buildinfo.Version=$version -X github.com/marcus/avatars/internal/buildinfo.Commit=$commit$dirty" -o "$temporary/$name" "./cmd/$name"
  done
  ...
  staged=
  for name in $commands; do
    staged_link=$bin_dir/.avatars-dev-$$-$name
    ln -s "$destination/$name" "$staged_link"
    staged="$staged $staged_link"
  done
  trap 'rm -f $staged' EXIT HUP INT TERM
  if command -v brew >/dev/null 2>&1 && brew list --versions avatars >/dev/null 2>&1; then
    brew unlink avatars >/dev/null
  fi
  clear_managed_links "$bin_dir"
  for name in $commands; do
    mv "$bin_dir/.avatars-dev-$$-$name" "$bin_dir/$name"
  done
  trap - EXIT HUP INT TERM
  printf 'activated local Avatars build from %s\n' "$repo_root"
  status
}

use_homebrew() {
  command -v brew >/dev/null 2>&1 || die "Homebrew is not installed"
  brew list --versions avatars >/dev/null 2>&1 || die "the avatars formula is not installed; run 'brew install marcus/tap/avatars'"
  bin_dir=$(active_bin_dir)
  preflight_switch "$bin_dir"
  previous=
  homebrew_count=0
  ...
  if [ "$homebrew_count" -eq 1 ]; then
    printf 'Homebrew Avatars is already active\n'
    status
    return
  fi
  for name in $commands; do
  ...
  if ! brew link --overwrite avatars >/dev/null; then
    brew unlink avatars >/dev/null 2>&1 || true
    for name in $commands; do
      path=$bin_dir/$name
  ...
  printf 'activated Homebrew Avatars\n'
  status
}
...
verify_homebrew() {
  use_homebrew
  brew update
  brew upgrade --yes marcus/tap/avatars
  brew test marcus/tap/avatars
  status
}
